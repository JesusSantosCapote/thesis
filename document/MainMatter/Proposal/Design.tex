...
definido, dicho tipo puede especificarse o no, un alias (\textbf{Alias}) que en caso de no ser vacío ser\'a el nombre del 
...
%Mira a ver que va primero este texto o el de arriba
...
es mediante algoritmo de busqueda de palabras clave (Keyword Search Algorithms, KSA), especificamente a 
...
con un grado interior de cero, implica que no hay otro subárbol que pueda subsumir los generados por dicha raíz.
...
contengas todos los nodos (tablas) de la consulta. Luego por cada \'arbol de join identificado 
...
de para dado una consulta obtener la lista de joins asociada.
...
inverso donde cada tabla (nodo) se enlaza a todos los árboles de unión que contienen 
...
tiene complejidad $O(|V| + |E|)$ empleando DFS tomando el reverso de los arcos y se realiza $Q$ veces. 
